...
<body class="min-h-screen flex flex-col relative overflow-x-hidden" style="background: linear-gradient(120deg,#11101a 60%,#23235b 100%);">
    ...
    <main class="flex-grow flex flex-col items-center justify-center px-4">
        <div class="w-full max-w-md rounded-xl shadow-2xl p-8" style="background: rgba(20, 18, 50, 0.7); backdrop-filter: blur(10px); border: 1px solid rgba(177, 185, 234, 0.2);">
            <h1 class="text-2xl md:text-3xl font-bold text-center mb-6" style="color: #b1b9ea; font-family: 'Poppins', sans-serif;">Connexion</h1>
            @if(session('error'))
                <div class="font-bold text-red-400 text-center mb-4">{{ session('error') }}</div>
            @endif
            <form action="{{ route('login') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input
                        ...
                        style="background: rgba(177, 185, 234, 0.1); border-color: rgba(177, 185, 234, 0.3); color: #b1b9ea; focus-ring-color: #5b6cb2;"
